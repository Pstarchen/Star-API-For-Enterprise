...
const acceptedMimeTypes = new Set(["image/png", "image/jpeg", "image/webp", "image/x-icon", "image/vnd.microsoft.icon"]);
...
export function parseIconDataUrl(value: string | undefined) {
  ...
  if (data.length === 0 || data.length > MAX_ICON_BYTES) throw new Error("INVALID_ICON_SIZE");
