...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error instanceof Error && error.message === "UPDATE_DISABLED") return Response.json({ code: 409, message: "尚未配置平台内更新令牌，请先在服务器配置 STAR_API_GITHUB_TOKEN_FILE" }, { status: 409, headers: noStoreHeaders });
    ...
    return Response.json({ code: 502, message: "无法提交更新任务，请检查 GitHub Actions 配置" }, { status: 502, headers: noStoreHeaders });
  }
}
