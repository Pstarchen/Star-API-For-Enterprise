...
export async function POST(request: Request) {
  ...
  try {
    await prisma.$transaction(async (transaction) => {
      ...
      const apiUpstream = await transaction.apiUpstream.create({ data: { productId: product.id, type: "PUBLIC_API", rewriteMode: upstream.pathname && upstream.pathname !== "/" ? "PREFIX" : "PASSTHROUGH", upstreamPrefix: upstream.pathname === "/" ? "" : upstream.pathname.replace(/\/$/, ""), healthPath: "/health", timeoutMs: 10000, authType: "NONE", nodes: { create: { name: "OpenAPI 主节点", baseUrl: upstream.origin, weight: 100 } } } });
