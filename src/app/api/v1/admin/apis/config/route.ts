...
import { inferPreparedDatasetContract, preparedContentResponseExample } from "@/lib/server/api-assets";
...
const updateSchema = z.object({
  ...
  route: z.object({ publicHost: hostSchema, publicPath: pathSchema, routeVersion: z.string().trim().min(1).max(24), methods: z.array(z.enum(apiHttpMethods)).min(1).max(apiHttpMethods.length).transform(normalizeMethods), requestFormat: z.enum(["JSON", "FORM", "BINARY", "ANY"]), responseFormats: z.array(z.enum(apiResponseFormats)).min(1).max(apiResponseFormats.length), corsEnabled: z.boolean(), forceHttps: z.boolean(), requestLogging: z.boolean(), dailyLimit: z.coerce.number().int().min(0).max(1_000_000_000), ipAllowlist: z.array(z.string().trim().min(1).max(64)).max(200), ipDenylist: z.array(z.string().trim().min(1).max(64)).max(200) }).strict(),
...
export async function GET(request: Request) {
  ...
  return Response.json({ code: 200, data: { id: product.id, name: product.name, slug: product.slug, status: product.status, categoryId: product.categoryId, category: product.category.name, visibility: product.visibility, billingMode: product.billingMode, unitPrice: product.unitPrice.toString(), freeQuotaMonthly: product.freeQuotaMonthly.toString(), defaultQpsLimit: product.defaultQpsLimit, route: endpoint ? { id: endpoint.id, publicHost: endpoint.publicHost, publicPath: endpoint.publicPath, routeVersion: endpoint.routeVersion, methods: endpoint.methods, requestFormat: endpoint.requestFormat, responseFormats: endpoint.responseFormats, responseExample: endpoint.responseExample, corsEnabled: endpoint.corsEnabled, forceHttps: endpoint.forceHttps, requestLogging: endpoint.requestLogging, dailyLimit: endpoint.dailyLimit.toString(), ipAllowlist: endpoint.ipAllowlist, ipDenylist: endpoint.ipDenylist } : null, upstream: product.upstream ? { id: product.upstream.id, type: product.upstream.type, rewriteMode: product.upstream.rewriteMode, upstreamPrefix: product.upstream.upstreamPrefix, healthPath: product.upstream.healthPath, healthStatus: product.upstream.healthStatus, timeoutMs: product.upstream.timeoutMs, authType: product.upstream.authType, secretConfigured: Boolean(product.upstream.secretConfigEncrypted), nodes: product.upstream.nodes.map((node) => ({ id: node.id, name: node.name, baseUrl: node.baseUrl, weight: node.weight, enabled: node.enabled, healthStatus: node.healthStatus, failureCount: node.failureCount, lastCheckedAt: node.lastCheckedAt?.toISOString() ?? null, lastError: node.lastError })) } : null, parameters: endpoint?.parameters.map((item) => ({ id: item.id, location: item.location, name: item.name, upstreamName: item.upstreamName ?? "", required: item.required, dataType: item.dataType, defaultValue: item.defaultValue ?? "", description: item.description, pattern: item.validation && typeof item.validation === "object" && !Array.isArray(item.validation) && "pattern" in item.validation ? String(item.validation.pattern ?? "") : "", sensitive: item.sensitive })) ?? [], responseParameters: endpoint?.responseParameters.map((item) => ({ id: item.id, name: item.name, dataType: item.dataType, description: item.description })) ?? [], dataset, audits: product.audits.map((audit) => ({ ...audit, createdAt: audit.createdAt.toISOString() })), assetCount: product.assets.length } }, { headers: noStoreHeaders });
}
...
export async function PATCH(request: Request) {
  ...
  const nextExecution = parsed.data.dataset ? { ...currentExecution, dataset: parsed.data.dataset } : currentExecution;
  ...
  const responseExample = generateResponseExample(responseParameters, parsed.data.route.responseFormats, contentSample);
  try {
    await prisma.$transaction(async (transaction) => {
      await transaction.apiProduct.update({ where: { id: product.id }, data: { status: "DRAFT", categoryId: category.id, visibility: parsed.data.visibility, billingMode: parsed.data.billingMode, unitPrice: parsed.data.billingMode === "FREE" ? 0 : parsed.data.unitPrice, freeQuotaMonthly: parsed.data.freeQuotaMonthly, defaultQpsLimit: parsed.data.defaultQpsLimit, ...(parsed.data.dataset ? { executionConfig: nextExecution as Prisma.InputJsonValue } : {}) } });
      await transaction.endpoint.update({ where: { id: endpoint.id }, data: { publicHost: parsed.data.route.publicHost, publicPath: parsed.data.route.publicPath, routeVersion: parsed.data.route.routeVersion, methods: parsed.data.route.methods, requestFormat: parsed.data.route.requestFormat, responseFormats: parsed.data.route.responseFormats, responseExample: responseExample as Prisma.InputJsonValue, corsEnabled: parsed.data.route.corsEnabled, forceHttps: parsed.data.route.forceHttps, requestLogging: parsed.data.route.requestLogging, dailyLimit: parsed.data.route.dailyLimit, ipAllowlist: parsed.data.route.ipAllowlist, ipDenylist: parsed.data.route.ipDenylist } });
      ...
      await transaction.auditLog.create({ data: { tenantId: auth.workspace?.tenantId, actorId: auth.user.id, action: "api.config.update", resource: "api-product", resourceId: product.id, metadata: { route: parsed.data.route.publicPath, nodes: parsed.data.upstream.nodes.length, parameters: parameters.length, contractMode: parsed.data.dataset?.contractMode }, ipAddress: requestIp(request) } });
    });
  } catch (error) {
