...
export async function POST(request: Request) {
  ...
  try {
    await prisma.$transaction(async (transaction) => {
      ...
      if (databaseAssets.length) await transaction.apiAsset.createMany({ data: databaseAssets.map((asset) => ({ productId: product.id, kind: asset.kind, name: asset.name, mimeType: asset.mimeType, data: asset.data, size: asset.size })) });
