...
export default async function InstallPage() {
  ...
  return <main className="min-h-screen bg-[var(--surface)] lg:grid lg:grid-cols-[380px_1fr]">
    <aside className="auth-art px-6 py-8 text-white sm:px-10 lg:min-h-screen lg:px-12 lg:py-12">
      <div className="rounded-[5px] bg-white p-1.5 inline-block"><BrandMark /></div>
      <p className="mono mt-12 text-[10px] text-[#6bdeb8]">SYSTEM INITIALIZATION</p>
      <h1 className="mt-4 text-2xl font-bold">配置你的 API 服务平台</h1>
      <div className="mt-10 space-y-1">
        {steps.map((step, index) => <div key={step.label} className="relative flex gap-4 py-4">
          ...
          <span className={`grid size-8 shrink-0 place-items-center rounded-[5px] ${step.done ? "bg-[#6bdeb8] text-[var(--night)]" : "border border-white/20 text-white/55"}`}>{step.done ? <Check className="size-4" /> : <step.icon className="size-4" />}</span>
    ...
    <section className="relative flex min-h-[70vh] items-center px-5 py-14 sm:px-10 lg:px-16"><ThemeToggle className="absolute right-5 top-5 grid size-9 place-items-center rounded-[5px] text-[var(--muted)] hover:bg-[var(--surface-subtle)]" />
