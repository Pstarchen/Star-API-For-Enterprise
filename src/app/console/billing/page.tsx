...
export default async function BillingPage() {
  ...
  const [usage, calls, invoices, orders, wallet, walletEntries, integrations] = workspace
    ? await Promise.all([
  ...
  return <div className="mx-auto max-w-[1200px] space-y-5">
    ...
    <WalletRechargeManager balance={wallet?.balance.toString() ?? "0.00"} entries={walletEntries.map((entry) => ({ id: entry.id, type: entry.type, delta: entry.delta.toString(), balanceAfter: entry.balanceAfter.toString(), reason: entry.reason, createdAt: entry.createdAt.toISOString() }))} orders={rechargeOrders.map((order) => ({ id: order.id, orderNo: order.orderNo, channel: order.channel, status: order.status, amount: order.amount.toString(), createdAt: order.createdAt.toISOString(), paidAt: order.paidAt?.toISOString() ?? null }))} channels={paymentChannels} />
    ...
    <PaymentManager invoices={invoices.filter((item) => item.status === "ISSUED").map((item) => ({ id: item.id, period: item.period, amount: item.amount.toString() }))} initialOrders={invoiceOrders.map((item) => ({ id: item.id, orderNo: item.orderNo, channel: item.channel, status: item.status, amount: item.amount.toString(), createdAt: item.createdAt.toISOString(), paidAt: item.paidAt?.toISOString() ?? null }))} channels={paymentChannels} />
  </div>;
}
