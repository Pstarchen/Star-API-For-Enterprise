...
export default async function ApiDetailPage({ params }: PageProps<"/apis/[slug]">) {
  ...
  const apiOrigin = process.env.API_PUBLIC_URL?.replace(/\/+$/, "") ?? `${new URL(platform.publicUrl).protocol}//${api.publicHost}`;
  const gatewayUrl = `${apiOrigin}${api.endpoint}`;
  ...
  return <PortalShell>
    ...
    <div className="container-shell grid gap-8 py-8 lg:grid-cols-[minmax(0,1fr)_260px]">
      <article className="min-w-0 space-y-8">
        ...
        <section><p className="eyebrow">SCHEMA</p><h2 className="mt-2 text-lg font-bold">端点 Schema</h2><pre className="mono mt-4 max-h-96 overflow-auto rounded-[7px] border border-[var(--line)] bg-[var(--surface)] p-4 text-[10px] leading-5"><code>{JSON.stringify(api.schema, null, 2)}</code></pre></section>
