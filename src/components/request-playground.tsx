...
export function RequestPlayground({ api, gatewayUrl }: { api: CatalogProduct; gatewayUrl: string }) {
  ...
  async function run(event: FormEvent) {
    ...
    try {
      ...
      const suffix = trimmedQuery ? (trimmedQuery.startsWith("?") ? trimmedQuery : `?${trimmedQuery}`) : "";
  ...
  function reset() {
    ...
    setQuery(""); setBody("{}"); setResponse(null); setError("");
  }
  ...
  return <div className="grid overflow-hidden rounded-[8px] border border-[var(--line-strong)] bg-[var(--line)] shadow-[var(--shadow-md)] lg:grid-cols-2">
    <form onSubmit={run} className="bg-[var(--surface)]">
      ...
      <div className="space-y-4 p-4 sm:p-5">
        ...
        <label className="block"><span className="mb-1.5 block text-[10px] font-semibold">查询参数</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="name=value&limit=10" className="mono h-10 w-full rounded-[6px] border border-[var(--line)] px-3 text-[10px] outline-none focus:border-[var(--brand)]" /></label>
        {!["GET", "HEAD"].includes(api.method) && <label className="block"><span className="mb-1.5 block text-[10px] font-semibold">JSON 请求体</span><textarea value={body} onChange={(event) => setBody(event.target.value)} rows={9} spellCheck={false} className="mono w-full rounded-[6px] border border-[var(--line)] bg-[var(--canvas)] p-3 text-[10px] leading-5 outline-none focus:border-[var(--brand)]" /></label>}
