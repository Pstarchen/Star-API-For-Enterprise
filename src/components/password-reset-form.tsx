...
export function ResetPasswordForm({ token }: { token: string }) {
  ...
  return <form onSubmit={submit} className="space-y-4"><FormField><FormLabel>新密码</FormLabel><Input name="password" type="password" required minLength={10} maxLength={72} autoComplete="new-password" placeholder="至少 10 位，包含字母和数字" /></FormField><FormField><FormLabel>确认新密码</FormLabel><Input name="confirmPassword" type="password" required minLength={10} maxLength={72} autoComplete="new-password" /></FormField>{error && <FormMessage>{error}</FormMessage>}<Button disabled={loading || !token} className="w-full">{loading && <Loader2 className="animate-spin" />}{loading ? "正在重置" : "重置密码"}</Button></form>;
}
