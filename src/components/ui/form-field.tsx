...
export function FormLabel({ className, ...props }: React.ComponentProps<"span">) {
  return <span data-slot="form-label" className={cn("block text-[13px] font-semibold text-[var(--ink)]", className)} {...props} />;
}

export function FormHint({ className, ...props }: React.ComponentProps<"span">) {
  return <span data-slot="form-hint" className={cn("block text-xs leading-5 text-[var(--muted)]", className)} {...props} />;
}
...
export function FormMessage({ className, tone = "error", ...props }: React.ComponentProps<"p"> & { tone?: "error" | "success" | "info" }) {
  ...
  return <p role={tone === "error" ? "alert" : "status"} data-slot="form-message" className={cn("rounded-[var(--radius-control)] border px-3 py-2.5 text-xs leading-5", tones[tone], className)} {...props} />;
}
