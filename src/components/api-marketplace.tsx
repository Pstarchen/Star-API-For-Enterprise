...
export function ApiMarketplace({ products, mode = "preview" }: { products: CatalogProduct[]; mode?: Mode }) {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((api) => (category === "全部" || api.category === category) && (method === "全部" || api.methods.includes(method)) && (billing === "全部" || (billing === "免费" ? api.billingMode === "FREE" : api.billingMode !== "FREE")) && (!keyword || [api.name, api.description, api.provider, api.endpoint, ...api.tags].join(" ").toLowerCase().includes(keyword))).sort((a, b) => sort === "调用最多" ? b.calls - a.calls : sort === "响应最快" ? (a.latency ?? Number.MAX_SAFE_INTEGER) - (b.latency ?? Number.MAX_SAFE_INTEGER) : Number(b.featured) - Number(a.featured));
  ...
  return <div className="market-page">
    ...
    <div className="container-shell py-6">
      ...
      <div className="market-toolbar"><div><strong>{filtered.length}</strong><span> 个匹配结果</span></div><div className="market-filters"><FilterSelect value={method} onValueChange={setMethod} options={["全部", "GET", "POST", "PUT", "PATCH", "DELETE"]} label="请求方法" /><FilterSelect value={billing} onValueChange={setBilling} options={["全部", "免费", "付费"]} label="计费方式" /><FilterSelect value={sort} onValueChange={setSort} options={["推荐", "调用最多", "响应最快"]} label="排序方式" /><div className="market-view-toggle"><Button type="button" onClick={() => setView("list")} variant={view === "list" ? "soft" : "ghost"} size="icon" className="size-10" aria-label="列表视图"><List /></Button><Button type="button" onClick={() => setView("grid")} variant={view === "grid" ? "soft" : "ghost"} size="icon" className="size-10" aria-label="网格视图"><LayoutGrid /></Button></div></div></div>
...
function FilterSelect({ value, onValueChange, options, label }: { value: string; onValueChange: (value: string) => void; options: string[]; label: string }) { return <Select value={value} onValueChange={onValueChange}><SelectTrigger className="w-28" aria-label={label}><SelectValue /></SelectTrigger><SelectContent>{options.map((option) => <SelectItem key={option} value={option}>{option}</SelectItem>)}</SelectContent></Select>; }
