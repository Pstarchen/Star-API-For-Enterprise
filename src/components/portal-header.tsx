      ...
      <div className="portal-bar">
        <div className="container-shell portal-nav">
          ...
          <div className="portal-island portal-actions-island relative hidden items-center gap-2 lg:flex">
            <Tooltip><TooltipTrigger asChild><ThemeToggle className="portal-action-control grid size-9 place-items-center text-[var(--muted)] transition hover:bg-[var(--surface-subtle)] hover:text-[var(--ink)]" /></TooltipTrigger><TooltipContent>切换深浅色模式</TooltipContent></Tooltip>
            ...
            </DropdownMenu> : <><Button asChild variant="ghost" size="sm" className="portal-action-control ml-1"><Link href="/login">登录</Link></Button><Button asChild size="sm" className="portal-action-control"><Link href="/register">免费注册</Link></Button></>}
          </div>
...
function MobileAccountLink({ href, icon: Icon, label, close }: { href: string; icon: typeof LayoutDashboard; label: string; close: () => void }) {
  return <Link href={href} onClick={close} className="flex h-10 items-center gap-2 rounded-[7px] px-2 text-xs transition hover:bg-[var(--surface-subtle)]"><Icon className="size-3.5" />{label}</Link>;
}
