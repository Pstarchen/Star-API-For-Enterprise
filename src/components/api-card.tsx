...
export function ApiCard({ api }: { api: ApiProduct }) {
  return (
    <Link
      href={`/apis/${api.slug}`}
      className="group flex min-h-[266px] flex-col border border-[var(--line)] bg-white p-5 transition-[border-color,box-shadow,transform] hover:-translate-y-0.5 hover:border-[var(--line-strong)] hover:shadow-[var(--shadow-md)] focus-visible:relative"
    >
      <div className="flex items-start justify-between gap-3">
        <span className="grid size-11 shrink-0 place-items-center rounded-[5px] text-[13px] font-bold text-white" style={{ backgroundColor: api.color }}>
          {api.shortName}
        </span>
