...
import { epayPaymentTypeNames, epayPaymentTypes, epayProtocolProfileNames, epaySubmissionModeNames, type EpayPaymentType, type EpayProtocolProfile, type EpaySubmissionMode } from "@/lib/payment-options";
...
export type PaymentProviderView = {
  ...
  paymentTypes: string[];
  protocolProfile: EpayProtocolProfile;
...
export function PaymentProvidersManager({ initialProviders }: { initialProviders: PaymentProviderView[] }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      paymentTypes,
      submissionMode: form.get("submissionMode"),
  ...
  return <div className="space-y-5">
    <header className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="eyebrow">EPAY PROVIDERS</p><h2 className="mt-1 text-xl font-bold">易支付服务商</h2><p className="mt-1 text-[11px] text-[var(--muted)]">pay.id0.cn 地址自动使用官方标准协议；连接测试会真实验证 PID、密钥和商户状态。</p></div><Button size="sm" onClick={() => { setEditing("NEW"); setError(""); }}><Plus />新增服务商</Button></header>
...
function Health({ provider }: { provider: PaymentProviderView }) { const variant = provider.healthStatus === "HEALTHY" ? "success" : provider.healthStatus === "UNHEALTHY" ? "danger" : "neutral"; const label = provider.healthStatus === "HEALTHY" ? "可用" : provider.healthStatus === "UNHEALTHY" ? "异常" : "未检测"; return <div><Badge variant={variant}>{label}</Badge><span className="mt-1 block max-w-56 truncate text-[9px] text-[var(--muted)]">{epayProtocolProfileNames[provider.protocolProfile]} · {epaySubmissionModeNames[provider.submissionMode]}</span><span className="mt-1 block max-w-56 truncate text-[9px] text-[var(--muted)]" title={provider.lastTestMessage ?? undefined}>{provider.lastTestMessage ?? "尚未执行商户验证"}</span></div>; }
